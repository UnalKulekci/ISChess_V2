...
def move_is_valid(player_order, move, board):
    ...
    def team_at(pos):
        col = color_at(pos)
        # Find the index of the color in the player_order string
        idx = player_order.find(col)
        if idx > 0:
            # The character before the color is the team ID
            return int(player_order[idx-1])
